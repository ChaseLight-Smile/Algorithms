...
分别用数组和链表存储数据，但是归并排序算法的思想精髓并没有因为存储数据的数据结构发生变化而变化。**需要强调的一点是，虽然采用什么样的数据结构，算法思想
不会改变，但是针对不同的数据结构，实现其算法的难度不一样，此时还是需要你进行仔细的选择**。在具体的实现中一定要区分清楚什么是算法思想以及数据怎么
...
　　很多算法都实现了递归版本和非递归版本，推荐使用非递归版本，因为递归很耗费空间，每一次递归都要开辟一个新的栈空间。代码可能会存在问题，如果发现，请修改并发起pull request。
